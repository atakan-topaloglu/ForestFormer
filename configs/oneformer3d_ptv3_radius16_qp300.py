...
# model settings
num_channels = 64  # PTV3 decoder outputs 64 channels
num_instance_classes = 3
num_semantic_classes = 3
radius = 16
grid_size = 0.2  # Voxel size for PTV3
...
model = dict(
    ...
    backbone=dict(
        ...
        dec_depths=(2, 2, 2, 2),
        dec_channels=(64, 64, 128, 256),  # First channel = output dim
    ...
    decoder=dict(
        ...
        in_channels=64,  # Must match PTV3 dec_channels[0]
        d_model=256,
        num_heads=8,
        hidden_dim=1024,
        dropout=0.0,
...
# Uniform LR for training from scratch
optim_wrapper = dict(
    type='OptimWrapper',
    optimizer=dict(type='AdamW', lr=0.001, weight_decay=0.05),
    clip_grad=dict(max_norm=10, norm_type=2))

# OneCycleLR scheduler (PTV3-style)
param_scheduler = [
    dict(
        type='OneCycleParamScheduler',
        param_name='lr',
        eta_max=0.006,
        total_steps=3000,     # REQUIRED for 1-cycle
        pct_start=0.05,
        anneal_strategy='cos',
        div_factor=10.0,
        final_div_factor=1000.0,
        by_epoch=True,        # or False if you schedule per iteration
    )
]
