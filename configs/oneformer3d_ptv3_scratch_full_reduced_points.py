"""
ForestFormer PTV3 - Training from Scratch (Full Dataset)

This config trains ForestFormer with PTV3 backbone from scratch (no pretrained weights)
using the full dataset with standard augmentation (no overfitting to a single sample).

Usage:
    python tools/train.py configs/oneformer3d_ptv3_scratch_full.py
"""
...
model = dict(
    ...
    backbone=dict(
        ...
        # Gradient checkpointing for memory efficiency (reduces activation memory)
...
# Dataloader settings - full dataset (no indices override)
