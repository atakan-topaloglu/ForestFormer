"""
ForestFormer with Point Transformer V3 Backbone - Training from Scratch

This config trains ForestFormer with PTV3 backbone from scratch (no pretrained weights).

Memory optimizations enabled:
- Gradient checkpointing: Enabled (reduces memory by ~40-50%, ~20-30% slower)
- Reduced patch sizes: 512 instead of 1024 (reduces attention memory)
- Reduced num_points: 400000 instead of 640000 (reduces input memory)

Note: The 'chunk' parameter is only available in ForAINetV2OneFormer3D_XAwarequery,
not in ForAINetV2OneFormer3D_PTV3. For PTV3, use the above optimizations instead.

Usage:
    python tools/train.py configs/oneformer3d_ptv3_scratch.py
"""
...
model = dict(
    ...
    backbone=dict(
        ...
        # Gradient checkpointing for memory efficiency - reduces memory by ~40-50%
...
train_pipeline = [
    ...
    dict(
        type='PointSample_',
        num_points=400000),  # Reduced from 640000 for memory efficiency
...
# Optimizer settings - slightly higher LR for training from scratch
optim_wrapper = dict(
    type='OptimWrapper',
