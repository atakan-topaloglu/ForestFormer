...
class ForAINetV2OneFormer3D_XAwarequery(Base3DDetector):
    ...
    #def predict_rm_outputpoints(self, batch_inputs_dict, batch_data_samples, **kwargs):
        ...
        if is_test:
            ...
            for region_idx, region in enumerate(tqdm(regions, desc="Processing regions")):
                ...
                #offset_ply_path = os.path.join(region_dir, "offset_logits.ply")
                #embed_ply_path = os.path.join(region_dir, "embed_logits.ply")
                #bi_semantic_ply_path = os.path.join(region_dir, "bi_semantic_logits.ply")
                #semantic_ply_path = os.path.join(region_dir, "semantic_logits.ply")

                #projected_offsets = offset_logits[inverse_mapping2]
                #projected_embed_logits = embed_logits[inverse_mapping2]
                #projected_bi_semantic_logits = bi_semantic_logits[inverse_mapping2]
                #projected_semantic_logits = semantic_logits[inverse_mapping2]
                
                #device=pc3_indices.device
                #originids = pc3_indices.cpu().numpy()  # Use pc3_indices for ground truth labels
                #pc3_instance_gt = pts_instance_gt[originids]
                #pc3_instance_gt = torch.from_numpy(pc3_instance_gt)
                #pc3_instance_gt = pc3_instance_gt.to(device)
                #voxel_superpoints = inverse_mapping2[coordinates[:, 0][inverse_mapping2] == 0]
                #voxel_instance_labels = self.get_voxel_instance_labels(
                #                        pc3_instance_gt, 
                #                        voxel_superpoints
                #                    )

                #current_points = original_points[originids]
                #current_points_add = scatter_add(current_points, voxel_superpoints, dim=0)
                ...
                #################################################################
                #distances = torch.norm(projected_offsets, dim=1)
                #scores_en = torch.exp(-distances)

                #originids = pc3_indices.cpu().numpy()  # Use pc3_indices for ground truth labels
                #pc3_instance_gt = pts_instance_gt[originids]
                #pc3_instance_gt = torch.from_numpy(pc3_instance_gt)

                #mean_cov_dict = compute_mean_cov(projected_embed_logits, pc3_instance_gt)
                #distances = compute_mahalanobis_distances(projected_embed_logits, pc3_instance_gt, mean_cov_dict)
                #scores_embed = compute_scores(distances)

                #mean_cov_dict = self.compute_mean_cov(embed_logits, voxel_instance_labels)
                #distances = self.compute_mahalanobis_distances(embed_logits, voxel_instance_labels, mean_cov_dict)
                #scores_embed = self.compute_scores(distances)

                #mean_per_label, unique_labels = self.compute_mean(embed_logits, voxel_instance_labels)
                #distances = self.compute_euclidean_distances(embed_logits, voxel_instance_labels, mean_per_label, unique_labels)
                #scores_embed = self.compute_scores_2(distances)

                #pc3_semantic_gt = pts_semantic_gt[originids]
                #pc3_semantic_gt = torch.from_numpy(pc3_semantic_gt)
                #pc3_semantic_gt = pc3_semantic_gt.to(device)
                #one_hot_sem_mask = torch.nn.functional.one_hot(pc3_semantic_gt, num_classes=self.num_classes).float()

                # Step 4.2: Aggregate one-hot semantic labels to voxel level
                #voxel_semantic_label_sum = scatter_add(one_hot_sem_mask, voxel_superpoints, dim=0)

                # Step 4.3: Find the majority class for each voxel by taking argmax over the summed one-hot labels
                #voxel_semantic_labels = voxel_semantic_label_sum.argmax(dim=1)

                #self.save_ply_with_logits(avg_points.cpu().numpy(), offset_logits.cpu().numpy(), 
                #                        embed_logits.cpu().numpy(), bi_semantic_logits.cpu().numpy(), 
                #                        semantic_logits.cpu().numpy(), voxel_instance_labels, voxel_semantic_labels, scores_embed, qscore_logits, offset_ply_path)


                #self.save_ply_with_logits(pc3.cpu().numpy(), projected_offsets.cpu().numpy(), 
                #                        projected_embed_logits.cpu().numpy(), projected_bi_semantic_logits.cpu().numpy(), 
                #                        projected_semantic_logits.cpu().numpy(), scores_en, scores_embed, offset_ply_path)

                #################################################################


                #########################################FPS sampling from xyz##################################
                '''
                batch_tensor_4 = torch.zeros(avg_points.size(0), dtype=torch.long).to(avg_points.device)  # Ensure batch_tensor on same device
                selected_indices_fps = fps(avg_points, batch_tensor_4, ratio=min(self.query_point_num / avg_points.size(0), torch.tensor([1.0]).to(avg_points.device)))
                query_points_mask = torch.isin(voxel_superpoints, selected_indices_fps)
                query_points_indices = torch.where(query_points_mask)[0]
                pc3_ins_label_qp = torch.unique(pc3_instance_gt[query_points_indices])
                pc3_ins_label_qp_set = set(pc3_ins_label_qp.cpu().numpy())
                # Calculate new instance labels that have not been covered before
                new_labels = pc3_ins_label_qp_set - covered_instance_labels_qp2
                # Update the covered instance labels set with new unique labels
                covered_instance_labels_qp2.update(new_labels)

                semantic_labels = voxel_semantic_labels[selected_indices_fps]

                # Count non-ground points (labels 1 and 2)
                non_ground_count = torch.sum((semantic_labels == 1) | (semantic_labels == 2)).item()
                
                # Calculate the ratio of non-ground points to total sampled points in the region
                total_sampled_count = len(semantic_labels)
                non_ground_ratio = non_ground_count / total_sampled_count

                # Accumulate the non-ground ratio for this region
                non_ground_ratios2.append(non_ground_ratio)
 
                pc3_ins_label_qp = pc3_ins_label_qp[pc3_ins_label_qp != 0]
                pc3_ins_label_total = torch.unique(pc3_instance_gt)
                pc3_ins_label_total = pc3_ins_label_total[pc3_ins_label_total != 0]
                #instance_cover_ratio = len(pc3_ins_label_qp)/len(pc3_ins_label_total)
                instance_cover_ratio = 1 if len(pc3_ins_label_total) == 0 else len(pc3_ins_label_qp) / len(pc3_ins_label_total)
                instance_cover_ratios2.append(instance_cover_ratio)
                '''
                ################################################################################################

                wood_class = 1
                #semantic_predictions = torch.argmax(semantic_logits, dim=1)  # Get predicted class per voxel
                semantic_predictions_bi = torch.argmax(bi_semantic_logits, dim=1)
                tree_indices = torch.where(semantic_predictions_bi == wood_class)[0]  #all voxel
                #wood_indices = torch.where(semantic_predictions == wood_class)[0]  #all voxels
                    
                if tree_indices.numel() > 1:
                    
                    # case 4: FPS from all tree points
                    batch_tensor_4 = torch.zeros(embed_logits[tree_indices].size(0), dtype=torch.long).to(embed_logits.device)  # Ensure batch_tensor on same device
                    topk_indices_4 = fps(embed_logits[tree_indices], batch_tensor_4, ratio=min(self.query_point_num / embed_logits[tree_indices].size(0), torch.tensor([1.0]).to(embed_logits.device)))
                    selected_indices_case4 = tree_indices[topk_indices_4]
                    ...
                    #query_points_mask = torch.isin(voxel_superpoints, selected_indices_case1)
                    #query_points_mask = torch.isin(voxel_superpoints, selected_indices_case4)
                    #query_points_indices = torch.where(query_points_mask)[0]
                    #pc3_ins_label_qp = torch.unique(pc3_instance_gt[query_points_indices])
                    #pc3_ins_label_qp_set = set(pc3_ins_label_qp.cpu().numpy())
                    # Calculate new instance labels that have not been covered before
                    #new_labels = pc3_ins_label_qp_set - covered_instance_labels_qp
                    # Update the covered instance labels set with new unique labels
                    #covered_instance_labels_qp.update(new_labels)

                    ##################################FPS sampling from xyz####################################
                    '''
                    semantic_labels = voxel_semantic_labels[selected_indices_case4]
                    ...
                    # Accumulate the non-ground ratio for this region
                    non_ground_ratios.append(non_ground_ratio)
                    ...
                    #instance_cover_ratio = len(pc3_ins_label_qp)/len(pc3_ins_label_total)
                    instance_cover_ratio = 1 if len(pc3_ins_label_total) == 0 else len(pc3_ins_label_qp) / len(pc3_ins_label_total)
                    instance_cover_ratios.append(instance_cover_ratio)
                    '''
                    ##################################################################################

                    x = self.decoder(x, queries)
                    results_list = self.predict_by_feat_test(x, inverse_mapping2, pc3, selected_indices_case4)
                    #results_list = self.predict_by_feat_test(x, inverse_mapping2, pc3, selected_indices_case1)

                    # Collect masks and their scores, process them immediately
                    '''for mask_idx, (mask, score) in enumerate(zip(results_list[0].pts_instance_mask[0], results_list[0].instance_scores)):
                        if score > 0.6:  # Use masks with score > 0.6
                            mask_pc1 = self.nearest_neighbor_mapping(pc1, pc3, mask)
                            mask_points = pc1_indices[mask_pc1].cpu().numpy()

                            # Update global instance mask and scores using vectorized operations
                            update_mask = (score > global_instance_scores[mask_points])
                            global_instance_scores[mask_points[update_mask]] = score
                            all_pre_ins[mask_points[update_mask]] = max_instance
                    ...
                    cylinder_current_semantic_pre = self.nearest_neighbor_mapping(pc1, pc3, results_list[0].pts_semantic_mask[0])
                    #cylinder_current_instance_pre = self.nearest_neighbor_mapping(pc1, pc3, results_list[0].pts_instance_mask[1]-1)
                    
                    originids = torch.where(region_mask)[0].cpu().numpy()  # Move to CPU before using np.where
                    all_pre_sem = self.vote_semantic_labels(all_pre_sem, originids, cylinder_current_semantic_pre)
                    #all_pre_ins, max_instance = self.region_merging(all_pre_ins, max_instance, cylinder_current_instance_pre, originids)

                    # Get gt labels for pc1
                    #pc1_semantic_gt = pts_semantic_gt[originids]
                    #pc1_instance_gt = pts_instance_gt[originids]
                    #region_dir = f"work_dirs/oneformer3d_1xb4_forainetv2/test_vis/region_{region_idx}"
                    #region_ply_path = os.path.join(region_dir, "pc1_ins_sem.ply")
                    #self.save_ply(pc1.cpu().numpy(), cylinder_current_semantic_pre, cylinder_current_instance_pre, region_ply_path, pc1_semantic_gt, pc1_instance_gt)
                    ...
                    #offset_ply_path = os.path.join(region_dir, "query.ply")
                    #select_q_idx = results_list[0].query_select_voxel_idx2
                    #select_q_idx_tensor = torch.tensor(select_q_idx, device=voxel_instance_labels.device)
                    #mask_label = torch.arange(1, select_q_idx_tensor.numel() + 1, device=voxel_instance_labels.device).view(select_q_idx_tensor.shape)
                    #####self.save_ply_with_logits(avg_points.cpu().numpy()[select_q_idx], offset_logits.cpu().numpy()[select_q_idx], 
                    #####                    embed_logits.cpu().numpy()[select_q_idx], bi_semantic_logits.cpu().numpy()[select_q_idx], 
                    #####                    semantic_logits.cpu().numpy()[select_q_idx], mask_label, voxel_semantic_labels[select_q_idx], scores_embed[select_q_idx], qscore_logits[select_q_idx], offset_ply_path)


                    # Save each mask to a separate .ply file
                    #select_q_idx = results_list[0].query_select_voxel_idx[final_valid_mask]
                    #for mask_idx, (mask, score) in enumerate(zip(results_list[0].pts_instance_mask[0], results_list[0].instance_scores)):
                    ...
                    #        mask_file_path = os.path.join(region_dir, f"mask{mask_idx}_{score:.2f}.ply")
                            #####self.save_ply_2(mask_points, np.full(mask_points.shape[0], mask_idx), mask_file_path)
                    #        mask_file_path = os.path.join(region_dir, f"mask{mask_idx}_{score:.2f}_querypoint.ply")
                    #        mask_points = np.expand_dims(avg_points.cpu().numpy()[select_q_idx[mask_idx]], axis=0) 
                            #####self.save_ply_2(mask_points, np.full(mask_points.shape[0], mask_idx), mask_file_path)
            ...
            # Calculate the total number of labels and covered labels 
            #all_instance_labels = {label for label in all_instance_labels if label != 0}       
            #total_labels = len(all_instance_labels) 
            #covered_instance_labels_qp = {label for label in covered_instance_labels_qp if label != 0}
            #covered_labels = len(covered_instance_labels_qp)


            #########################################FPS sampling from xyz##################################
            '''
            covered_instance_labels_qp2 = {label for label in covered_instance_labels_qp2 if label != 0}
            covered_labels2 = len(covered_instance_labels_qp2)
            coverage_rate2 = covered_labels2 / total_labels

            average_non_ground_ratio = sum(non_ground_ratios) / len(non_ground_ratios)
            average_non_ground_ratio2 = sum(non_ground_ratios2) / len(non_ground_ratios2)

            average_instance_cover_ratios = sum(instance_cover_ratios) / len(instance_cover_ratios)
            average_instance_cover_ratios2 = sum(instance_cover_ratios2) / len(instance_cover_ratios2)
            '''
            #########################################FPS sampling from xyz##################################


            # Calculate the coverage rate
            #coverage_rate = covered_labels / total_labels

            #region_dir = os.path.join(output_path, current_filename)
            #region_ply_path = os.path.join(region_dir, "coverage_rate_qp.txt")

            # Write the coverage rate to the file
            #with open(region_ply_path, 'w') as file:
            #    file.write(f"Coverage rate by query selection: {coverage_rate:.2%}\n")
            #    file.write(f"Covered labels by query selection: {covered_labels}\n")
            #    file.write(f"Coverage rate by fps: {coverage_rate2:.2%}\n")
            #   file.write(f"Covered labels by fps: {covered_labels2}\n")
            #    file.write(f"Total labels: {total_labels}\n") 
            #    file.write(f"average_non_ground_ratio by query selection: {average_non_ground_ratio}\n") 
            #    file.write(f"average_non_ground_ratio by fps: {average_non_ground_ratio2}\n") 
            #    file.write(f"average_instance_cover_ratios by query selection: {average_instance_cover_ratios}\n") 
            #    file.write(f"average_instance_cover_ratios by fps: {average_instance_cover_ratios2}\n")         
            ...
            # Merge masks with IoU > 0.8  bm1/bm2
            #clean_all_pre_ins, merged_masks = self.merge_overlapping_instances(all_pre_ins, unique_best_masks)
            ...
            # Re-label instances to ensure continuous labeling
            #unique_labels = np.unique(all_pre_ins)
            #unique_labels = unique_labels[unique_labels >= 0]  # Exclude background label (-1)
            #relabel_map = {old_label: new_label for new_label, old_label in enumerate(unique_labels)}
            #relabel_map[-1] = -1  # Keep background as -1
            #all_pre_ins = np.vectorize(relabel_map.get)(all_pre_ins)

            # Save the final combined results
            #region_path = f"work_dirs/oneformer3d_1xb4_forainetv2_new/test_vis_score08_0806_bmscorebasedv2/{current_filename}_final_results_beforeclean.ply"
            #self.save_ply_withscore(original_points.cpu().numpy(), final_semantic_labels, all_pre_ins, global_instance_scores, region_path, pts_semantic_gt, pts_instance_gt)
            ...
            #batch_data_samples[0].eval_ann_info['pts_instance_mask']
            #batch_data_samples[0].eval_ann_info['pts_semantic_mask']  #nparray
            #batch_inputs_dict['points'][0][0:300]
            return batch_data_samples
    ...
    @staticmethod
    def vote_semantic_labels(all_pre_sem, originids, cylinder_current_semantic_pre):
        # 检查 originids 是否为标量或单元素数组
        if hasattr(originids, "ndim"):
            if originids.ndim == 0 or (originids.ndim == 1 and len(originids) == 1):  # 单元素数组
                originids = [originids.item()]
        elif not hasattr(originids, "__iter__"):  # 如果不是可迭代对象
            originids = [originids]

        # 检查 cylinder_current_semantic_pre 是否为标量或单元素张量
        if hasattr(cylinder_current_semantic_pre, "ndim"):
            if cylinder_current_semantic_pre.ndim == 0 or (cylinder_current_semantic_pre.ndim == 1 and len(cylinder_current_semantic_pre) == 1):  # 单元素张量
                cylinder_current_semantic_pre = [cylinder_current_semantic_pre.item()]
        elif not hasattr(cylinder_current_semantic_pre, "__iter__"):  # 如果不是可迭代对象
            cylinder_current_semantic_pre = [cylinder_current_semantic_pre]

        for idx, originid in zip(originids, cylinder_current_semantic_pre):
            all_pre_sem[idx].append(originid.item() if hasattr(originid, "item") else originid)
